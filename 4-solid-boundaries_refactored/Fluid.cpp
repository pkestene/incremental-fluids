...
public:
	...
	for (int x = 1; x < _w - 1; x++) {
	  ...
	  _mask_map[idx] = 0;
	  if (nx != 0.0 && _cell[idx + sgn(nx)]    != CELL_FLUID)
	    _mask_map[idx] |= 1; /* Neighbour in normal x direction is blocked */
	  if (ny != 0.0 && _cell[idx + sgn(ny)*_w] != CELL_FLUID)
	    _mask_map[idx] |= 2; /* Neighbour in normal y direction is blocked */
	...
	printf("mask map size before %d\n",_mask_map.size());
	//while (_mask_map.size() > 0) {

	for (auto iter = _mask_map.begin(); iter != _mask_map.end(); ) {
	    int idx = iter->first;
	    int y = idx/_w;
	    int x = idx-_w*y;
	    ...
	    if (_cell[idx] != CELL_FLUID && _mask[idx] == 0) {
	      ...
	      _mask_map.erase(iter++);
	      
	      printf("computing cell %d %d - src is %f\n",x,y,_src[idx]);
	    } else {
	      iter++;
	    }
	    
	}
	printf("mask map size after %d\n",_mask_map.size());

...
class FluidQuantity {
  ...
  std::unordered_map<int,uint8_t> _mask_map;
...
public:
  ...
  void fillSolidMask() {
    ...
    //printf("size=%d nbReady=%d nbTodo=%d\n",_mask_map.size(),nbReady,nbTodo);
  ...
  // return true if at least 2 neighbors are in DONE state, meaning all upwind neighbor
  // have already been computed, and meaning that current cell can flip in READY
  bool is_ready_to_compute(int idx, std::unordered_map<int,uint8_t>& umap) {
    int y = idx/_w;
    int x = idx-_w*y;

    int nbDone=0;
    if (_cell[idx-1] != CELL_FLUID && umap[idx-1] == DONE)
      nbDone++;
    if (_cell[idx+1] != CELL_FLUID && umap[idx+1] == DONE)
      nbDone++;
    if (_cell[idx-_w] != CELL_FLUID && umap[idx-_w] == DONE)
      nbDone++;
    if (_cell[idx+_w] != CELL_FLUID && umap[idx+_w] == DONE)
      nbDone++;
  ...
  int get_number_of_cells_per_state(std::unordered_map<int,uint8_t>& umap, CellMaskState state) {
    
    int nbState = 0;
    for (const auto& iter : _mask_map) {
  ...
  void extrapolate() {
    ...
    for (auto iter = _mask_map.begin(); iter != _mask_map.end(); ++iter) {
      int idx = iter->first;
      int y = idx/_w;
      int x = idx-_w*y;
      
      if (_cell[idx] != CELL_FLUID && iter->second==READY) {
    ...
    // as long as there are solid cell to compute, sweep the mask map for cell to
    // extrapolate (in TODO state)
    int nbTodo  = get_number_of_cells_per_state(_mask_map,TODO);
	...
	if (_cell[idx] != CELL_FLUID && iter->second != DONE && is_ready_to_compute(idx, _mask_map)) {
    ...
    //printf("nb TODOs = %d\n",get_number_of_cells_per_state(_mask_map,TODO));
    //printf("nb READYs = %d\n",get_number_of_cells_per_state(_mask_map,READY));
    //printf("nb DONEs = %d\n",get_number_of_cells_per_state(_mask_map,DONE));
